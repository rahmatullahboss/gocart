                ...
                <div className="flex flex-col gap-3 mt-14">
                    {(Array.isArray(product.rating) ? product.rating : []).map((item,index) => (
                        <div key={index} className="flex gap-5 mb-10">
                            <Image src={item.user?.image || '/favicon.ico'} alt="" className="size-10 rounded-full" width={100} height={100} />
                            <div>
                                <div className="flex items-center" >
                                    {Array(5).fill('').map((_, index) => (
                                        <StarIcon key={index} size={18} className='text-transparent mt-0.5' fill={(item.rating || 0) >= index + 1 ? "#00C950" : "#D1D5DB"} />
                                    ))}
                                </div>
